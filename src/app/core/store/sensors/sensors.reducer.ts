...
export const sensorsReducer = createReducer(
  ...
  on(SensorsActions.loadDevicesSuccess, (state, { devices }) => ({ ...state, loading: false, devices })),
  on(SensorsActions.loadDevicesFailure, (state, { error }) => ({ ...state, loading: false, error })),
  on(SensorsActions.loadReadings, (state) => ({ ...state, loading: true })),
  on(SensorsActions.loadReadingsSuccess, (state, { readings }) => ({ ...state, loading: false, readings })),
  on(SensorsActions.loadReadingsFailure, (state, { error }) => ({ ...state, loading: false, error })),
  on(SensorsActions.loadSummary, (state) => ({ ...state, loading: true })),
  on(SensorsActions.loadSummarySuccess, (state, { summary }) => ({ ...state, loading: false, summary })),
  on(SensorsActions.loadSummaryFailure, (state, { error }) => ({ ...state, loading: false, error })),
