...
@Component({
  ...
  standalone: true,
  imports: [NgIf, NgFor, NgSwitch, NgSwitchCase, NgSwitchDefault, RouterLink, NumberAbbreviatePipe, DateFormatPipe, StellarAddressPipe, DataTableComponent, StatusBadgeComponent, LucideAngularModule],
  template: `
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Admin Dashboard</h1>
          <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">System overview and management</p>
        </div>
      </div>
      ...
      <ng-container *ngIf="!loading">
          ...
          <div class="card p-5">
            <div class="flex items-center justify-between mb-3">
              ...
              <div class="w-9 h-9 rounded-lg bg-stellar-blue/10 flex items-center justify-center">
                <lucide-angular [img]="UsersIcon" class="w-4 h-4 text-stellar-blue"></lucide-angular>
              </div>
            </div>
            <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ (overview?.totalUsers ?? 0) | numberAbbreviate }}</p>
          </div>

          <div class="card p-5">
            <div class="flex items-center justify-between mb-3">
              <p class="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Projects</p>
              <div class="w-9 h-9 rounded-lg bg-environmental-green/10 flex items-center justify-center">
                <lucide-angular [img]="Building2" class="w-4 h-4 text-environmental-green"></lucide-angular>
              </div>
            </div>
            <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ (overview?.totalProjects ?? 0) | numberAbbreviate }}</p>
            <p class="text-xs text-green-600 mt-1">{{ overview?.activeProjects ?? 0 }} active</p>
          </div>

          <div class="card p-5">
            <div class="flex items-center justify-between mb-3">
              <p class="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Oracles</p>
              <div class="w-9 h-9 rounded-lg bg-credit-gold/10 flex items-center justify-center">
                <lucide-angular [img]="ShieldCheck" class="w-4 h-4 text-credit-gold"></lucide-angular>
              </div>
            </div>
            <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ (overview?.verifiedOracles ?? 0) | numberAbbreviate }}</p>
          </div>

          <div class="card p-5">
            <div class="flex items-center justify-between mb-3">
              <p class="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Retirements</p>
              <div class="w-9 h-9 rounded-lg bg-retirement-red/10 flex items-center justify-center">
                <lucide-angular [img]="RefreshCw" class="w-4 h-4 text-retirement-red"></lucide-angular>
              </div>
            </div>
            <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ (overview?.totalRetirements ?? 0) | numberAbbreviate }}</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div class="card p-5">
            ...
            <div class="space-y-3">
              <div class="flex items-center justify-between py-2 border-b border-slate-100 dark:border-slate-700">
                ...
                <span class="text-sm font-semibold text-yellow-600 dark:text-yellow-400">{{ queueDepths.pendingSubmissions }}</span>
              </div>
              <div class="flex items-center justify-between py-2 border-b border-slate-100 dark:border-slate-700">
                <span class="text-sm text-slate-600 dark:text-slate-400">Active Retirements</span>
                <span class="text-sm font-semibold text-stellar-blue">{{ queueDepths.activeRetirements }}</span>
              </div>
              <div class="flex items-center justify-between py-2">
                <span class="text-sm text-slate-600 dark:text-slate-400">Pending Verifications</span>
                <span class="text-sm font-semibold text-environmental-green">{{ queueDepths.pendingVerifications }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="card p-5">
            ...
            <div class="space-y-2">
              <a routerLink="/admin/oracles" class="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <p class="text-sm font-medium text-slate-700 dark:text-slate-300">Oracle Management</p>
                    <p class="text-xs text-slate-400">Manage oracle nodes and submissions</p>
                  </div>
                </div>
                <lucide-angular [img]="ArrowRight" class="w-4 h-4 text-slate-300 group-hover:text-slate-500 transition-colors"></lucide-angular>
              </a>
              <a routerLink="/admin/fees" class="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
                <div class="flex items-center gap-3">
                  <div class="w-8 h-8 rounded-lg bg-stellar-blue/10 flex items-center justify-center">
                    <lucide-angular [img]="Settings" class="w-4 h-4 text-stellar-blue"></lucide-angular>
                  </div>
                  <div>
                    <p class="text-sm font-medium text-slate-700 dark:text-slate-300">Fees & Parameters</p>
                    <p class="text-xs text-slate-400">Configure protocol fees and thresholds</p>
                  </div>
                </div>
                <lucide-angular [img]="ArrowRight" class="w-4 h-4 text-slate-300 group-hover:text-slate-500 transition-colors"></lucide-angular>
              </a>
              <a routerLink="/admin/users" class="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
                <div class="flex items-center gap-3">
                  <div class="w-8 h-8 rounded-lg bg-environmental-green/10 flex items-center justify-center">
                    <lucide-angular [img]="UsersIcon" class="w-4 h-4 text-environmental-green"></lucide-angular>
                  </div>
                  <div>
                    <p class="text-sm font-medium text-slate-700 dark:text-slate-300">User Management</p>
        ...
        <div class="card p-5">
          <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-4">Recent Submissions</h3>
          <app-data-table
