...
@Component({
  ...
  template: `
    <div class="max-w-3xl mx-auto">
      <a routerLink="/projects" class="inline-flex items-center gap-1 text-sm text-stellar-blue hover:text-stellar-blue-light mb-6">
        <lucide-angular [img]="ChevronLeft" class="w-4 h-4"></lucide-angular>
        Back to Projects
      </a>

      <div class="card p-6">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white mb-2">{{ isEdit ? 'Edit Project' : 'New Project' }}</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">Fill in the details to {{ isEdit ? 'update your' : 'register a new' }} water credit project.</p>
        ...
        <ng-container [ngSwitch]="currentStep">
          <div *ngSwitchCase="0" class="space-y-4">
            <div>
              ...
              <input type="text" [(ngModel)]="form.name" class="input" placeholder="e.g., Clear Water Valley Restoration" required />
            </div>
            <div>
              <label class="label">Description *</label>
              <textarea [(ngModel)]="form.description" class="input min-h-[100px]" placeholder="Describe the project location, methods, and expected impact..." required></textarea>
            </div>
            <div>
          ...
          <div *ngSwitchCase="1" class="space-y-4">
            ...
            <div>
              ...
              <input type="number" [(ngModel)]="form.areaHectares" class="input" placeholder="e.g., 100" min="0" step="0.01" />
            </div>
          </div>
          ...
          <div *ngSwitchCase="3" class="space-y-3">
            <div class="bg-slate-50 dark:bg-dark-bg rounded-lg p-4 space-y-3">
              ...
              <div class="grid grid-cols-2 gap-3 text-sm">
                <div><span class="text-slate-400">Name:</span> <span class="font-medium">{{ form.name }}</span></div>
                <div><span class="text-slate-400">Methodology:</span> <span class="font-medium">{{ form.methodology }}</span></div>
                <div><span class="text-slate-400">Area:</span> <span class="font-medium">{{ form.areaHectares }} ha</span></div>
                <div><span class="text-slate-400">Location:</span> <span class="font-medium">{{ form.latitude.toFixed(4) }}, {{ form.longitude.toFixed(4) }}</span></div>
                <div><span class="text-slate-400">Baseline:</span> <span class="font-medium">{{ form.baselineStart }} to {{ form.baselineEnd }}</span></div>
              </div>
        ...
        <div class="flex justify-between mt-8 pt-6 border-t border-slate-200 dark:border-slate-700">
          ...
          <div class="flex gap-3 ml-auto">
            <button *ngIf="currentStep < 3" (click)="nextStep()" [disabled]="!canProceed" class="btn btn-primary">Next</button>
            <button *ngIf="currentStep === 3" (click)="save()" [disabled]="saving" class="btn btn-primary flex items-center gap-2">
...
export class ProjectFormComponent implements OnInit {
  ...
  protected steps: FormStep[] = [
    { label: 'Basic Info', icon: ClipboardList, description: 'Provide the basic information about your project.' },
    { label: 'Location', icon: MapPin, description: 'Set the geographic location and size of your project.' },
  ...
  get canProceed(): boolean {
    switch (this.currentStep) {
      case 0: return !!this.form.name && !!this.form.description && !!this.form.methodology;
      case 1: return !!this.form.latitude && !!this.form.longitude && this.form.areaHectares > 0;
      case 2: return !!this.form.baselineStart && !!this.form.baselineEnd;
      default: return true;
    }
  }
