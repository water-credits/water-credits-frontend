  ...
  template: `
    <div class="bg-white dark:bg-dark-bg-lighter rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
      <div class="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-700">
        ...
        <button (click)="reset.emit()" class="text-xs text-stellar-blue hover:text-stellar-blue-light flex items-center gap-1">
      ...
      <div class="p-4 space-y-4">
        <div *ngFor="let filter of filters" class="space-y-1.5">
          ...
          <select *ngIf="filter.type === 'select'" [ngModel]="filter.value" (ngModelChange)="onFilterChange(filter.key, $event)" class="input">
            <option value="">All</option>
            <option *ngFor="let opt of filter.options" [value]="opt.value">{{ opt.label }}</option>
          </select>
          <input *ngIf="filter.type === 'text'" [ngModel]="filter.value" (ngModelChange)="onFilterChange(filter.key, $event)" type="text" class="input" placeholder="Filter by {{ filter.label.toLowerCase() }}..." />
          <div *ngIf="filter.type === 'range'" class="flex items-center gap-2">
            <input [ngModel]="filter.value?.min" (ngModelChange)="onRangeChange(filter.key, 'min', $event)" type="number" class="input" placeholder="Min" />
            <span class="text-slate-400">-</span>
            <input [ngModel]="filter.value?.max" (ngModelChange)="onRangeChange(filter.key, 'max', $event)" type="number" class="input" placeholder="Max" />
          </div>
        </div>
      </div>
